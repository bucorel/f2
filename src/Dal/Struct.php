...
class Struct{
	...
	function set( $fieldName, $value ){
		if( !array_key_exists( $fieldName, $this->fields ) ){
			throw new StructException( "UNKNOWN_FIELD", $fieldName );
		}
	...
	private function setMultiValue( $fieldName, $value ){
		if( !is_array( $value ) ){
			throw new StructException( "ARRAY_EXPECTED", $fieldName );
		}
		...
		try{
			...
			throw new StructException( $e->getMessage(), $fieldName );
		}
	...
	public static function validateStruct( $value ){
		...
		throw new \Exception( "STRUCTERR_INVALID_STRUCT" );
	}
	...
	public function setKey(string $fieldName, string $key, mixed $value ){
		if( !array_key_exists( $fieldName, $this->fields ) ){
			throw new StructException( "STRUCTERR_UNKNOWN_FIELD", $fieldName );
		}
		
		if( $this->fields[ $fieldName ][0] != self::TYPE_KEYVAL ){
			throw new StructException( "STRUCTERR_NOT_A_KEYVAL", $fieldName );
		}
		
		$this->data[ $fieldName ][ $key ] = $value;
	}
	
	public function getKey( string $fieldName, string $key ){
		if( !array_key_exists( $fieldName, $this->fields ) ){
			throw new StructException( "STRUCTERR_UNKNOWN_FIELD", $fieldName );
		}
		...
		if( !isset( $this->data[ $fieldName ][ $key ] ) ){
			throw new StructException( 'STRUCTERR_UNKNOWN_KEY ('.$key.')',$fieldName );
		}
